/* JBS 共通スクリプト（依存ライブラリなし）
   1. ドロワーナビ（≤900px）  2. サブメニュー開閉  3. ヒーロー写真の回転
   4. コンテンツ一覧のアコーディオン（≤900px） */
(function () {
  'use strict';
  ...
  if (burger && gnav) {
    ...
    if (mqSp.addEventListener) { mqSp.addEventListener('change', onChange); } else { mqSp.addListener(onChange); }
  }
...
(function () {
  ...
  // 「視差効果を減らす」設定のときは自動送りしない
  var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (!reduce) { start(); }
})();

/* --- 4. コンテンツ一覧のアコーディオン（2026-09-08 ヒートマップ反映 G） -------- */
(function () {
  'use strict';
  var tgls = document.querySelectorAll('.jbs-hub__col .jbs-hub__tgl');
  Array.prototype.forEach.call(tgls, function (tgl) {
    var col = tgl.closest('.jbs-hub__col');
    if (!col) { return; }
    tgl.addEventListener('click', function () {
